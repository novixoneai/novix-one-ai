...
Upload `novix-one-hostinger-upload.zip` to the domain's document root in Hostinger hPanel, usually `public_html/`, and extract it there. The extracted `index.html` must be directly inside `public_html/`—not inside a nested folder.
...
4. Open the document root for `novixone.co`.
5. Upload `novix-one-hostinger-upload.zip`.
6. Extract it in the document root.
7. Confirm that `index.html`, `assets/`, `images/`, `novix-hero.mp4`, and `favicon.svg` are directly in that folder.
...
After changing the site, run `bun run build`, replace the contents of the Hostinger package with the new `dist/` contents, create a new ZIP with the same structure, and upload/extract it into the document root.
